        ...
        public Image GetBarcode(string selectedBarcode, string barCodeLabel, int width, int height)
        {
            ...
            var label = Regex.Replace(Regex.Replace(barCodeLabel, "<BAR|\\++>|>", ""),@"\D","0");
                switch (selectedBarcode)
                {
                    ...
                    return _barcode.Encode(BarcodeLib.TYPE.CODE39, label.Length==0?"0":label, Color.Black, Color.White, width, height);
                    case "Code128":
                    _barcode.IncludeLabel = true;
                    _barcode.LabelPosition = BarcodeLib.LabelPositions.BOTTOMCENTER;
                    return _barcode.Encode(BarcodeLib.TYPE.CODE128, label.Length == 0 ? "0" : label, Color.Black, Color.White, width, height);
                    case "EAN13":
                        {
                            if (label.Length < 13)
                            {
                            ...
                            _barcode.IncludeLabel = true;
                            _barcode.LabelPosition = BarcodeLib.LabelPositions.BOTTOMCENTER;
                            return _barcode.Encode(BarcodeLib.TYPE.EAN13, label.Length == 0 ? "0" : label, Color.Black, Color.White, width, height);
                        }
                    case "EAN8":
                        {
                            if (label.Length < 8)
                            ...
                            _barcode.IncludeLabel = true;
                            return _barcode.Encode(BarcodeLib.TYPE.EAN8, label.Length == 0 ? "0" : label, Color.Black, Color.White, width, height);
                        }
                    case "2/5 Interleaved":
                        {
                                ...
                                return _barcode.Encode(BarcodeLib.TYPE.Interleaved2of5, label.Length == 0 ? "0" : label, Color.Black, Color.White, width, height);
                        }
                    case "DataMatrix":
                        {
